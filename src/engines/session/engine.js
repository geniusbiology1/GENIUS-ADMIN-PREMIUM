export const SESSION_STATUS={UPCOMING:'UPCOMING',OPEN:'OPEN',COMPLETED:'COMPLETED',CANCELLED:'CANCELLED'};
export function openSession(session){return {...session,status:SESSION_STATUS.OPEN,openedAt:new Date().toISOString()};}
export function completeSession(session){return {...session,status:SESSION_STATUS.COMPLETED,completedAt:new Date().toISOString()};}
