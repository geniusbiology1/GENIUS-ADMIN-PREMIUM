...
import {daySessions} from './core.jsx';
...
function MiniBarChart({labels,seriesA,seriesB,colorA='var(--accent)',colorB='var(--muted-2)',legendA,legendB}){
 ...
 return <svg viewBox={`0 0 ${w} ${h}`} width="100%" height="140" aria-label="مخطط الإيرادات مقابل المصروفات">
  {labels.map((l,i)=>{
...
export function Reports(p){
 const [type,setType]=useState('students');const [branch,setBranch]=useState('ALL');const [from,setFrom]=useState('');const [to,setTo]=useState('');
 ...
 return <Screen title="التقارير والتحليلات" action={<div className="actions"><button className="btn" onClick={()=>p.exportXlsx(`GENIUS_REPORT_${today()}.xlsx`,{تقرير:build()})}>Excel</button><button className="btn secondary" onClick={()=>window.print()}>PDF / طباعة</button></div>}>
