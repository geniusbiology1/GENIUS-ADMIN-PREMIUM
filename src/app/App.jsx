...
function MainApp(){
 ...
 const write=useCallback(async(store,row,label)=>{const next={...row,updatedAt:new Date().toISOString()};await put(store,next);const outbox={id:uid('outbox'),createdAt:new Date().toISOString(),store,operation:'UPSERT',recordId:next.id,payload:next,status:'PENDING',attempts:0};await put('outbox',outbox);setData(x=>({...x,[store]:[...x[store].filter(r=>r.id!==next.id),next],outbox:[...(x.outbox||[]),outbox].slice(-1000)}));const act={id:uid('act'),at:new Date().toISOString(),action:label||`تعديل ${store}`,refId:next.id,store};await put('activities',act);setData(x=>({...x,activities:[...x.activities,act].slice(-500)}));return next},[]);
 const softDelete=useCallback(async(store,id,label)=>{if(!window.confirm('تأكيد الأرشفة؟ لن يتم حذف البيانات نهائيًا ويمكن استرجاعها.'))return;const row=data[store].find(x=>x.id===id);if(!row)return;await write(store,{...row,deletedAt:new Date().toISOString(),status:row.status==='نشط'?'مؤرشف':row.status},label||`أرشفة ${store}`);notify('تمت الأرشفة ويمكن الاسترجاع');await buzz()},[data,write,notify,buzz]);
 const filtered=useMemo(()=>{const visible=s=>active(s)&&(!yearId||s.academicYearId===yearId||(!s.academicYearId&&s.storeIndependent))&&(!branchId||branchId==='ALL'||s.branchId===branchId||(!s.branchId&&s.storeIndependent));return {groups:(data?.groups||[]).filter(visible),students:(data?.students||[]).filter(visible),sessions:(data?.sessions||[]).filter(visible),books:(data?.books||[]).filter(visible)}},[data,yearId,branchId]);
 ...
 useEffect(()=>{if(!data||locked)return;let remove;import('@capacitor/app').then(({App})=>{App.addListener('backButton',({canGoBack})=>{if(modal){setModal(null);return}if(page!=='dashboard'){setPage('dashboard');return}if(canGoBack)history.back();else App.exitApp()}).then(x=>{remove=()=>x.remove()})}).catch(()=>{});return()=>remove?.()},[data,locked,modal,page]);
 useEffect(()=>{configureNative().catch(()=>{})},[]);
 useEffect(()=>{if(!data||locked||!settings.notificationsEnabled)return;syncRuleNotifications(data).then(rows=>{if(rows.length)setData(x=>({...x,notifications:[...(x.notifications||[]),...rows]}))}).catch(()=>{})},[data,locked,settings.notificationsEnabled]);

 if(!data)return <div className="splash"><img src="/resources/splash.png" alt="GENIUS ADMIN PREMIUM"/><b>GENIUS ADMIN PREMIUM</b><small>Offline First • جاري تجهيز النظام...</small></div>;
 ...
 return <div className={`app ${theme}`} style={{'--accent':settings.accent||'#FF0000'}}>
  ...
  <nav>{NAV.slice(0,5).map(([id,label,Icon])=><button key={id} className={page===id?'active':''} onClick={()=>go(id)}><Icon/><span>{label}</span></button>)}</nav>
  {toast&&<div className="toast">{toast}</div>}
  {modal==='menu'&&<Modal title="كل وظائف GENIUS ADMIN" close={()=>setModal(null)}><div className="menuGrid">{[...NAV,['notifications','الإشعارات',I.Bell],['activity','سجل العمليات',I.History],['archive','الأرشيف',I.Archive]].map(([id,l,Icon])=><button key={id} onClick={()=>{setModal(null);go(id)}}><Icon/>{l}</button>)}</div></Modal>}
  {modal==='student'&&<StudentForm {...pageProps}/>} {modal==='studentView'&&selected&&<Student360 {...pageProps} student={selected}/>} {modal==='card'&&selected&&<StudentCard {...pageProps} student={selected}/>} {modal==='group'&&<GroupForm {...pageProps}/>} {modal==='session'&&<SessionForm {...pageProps}/>} {modal==='attendance'&&selected&&<AttendanceModal {...pageProps} session={selected}/>} {modal==='payment'&&<PaymentForm {...pageProps}/>} {modal==='expense'&&<ExpenseForm {...pageProps}/>} {modal==='exam'&&<ExamForm {...pageProps}/>} {modal==='examView'&&selected&&<ExamView {...pageProps} exam={selected}/>} {modal==='book'&&<BookForm {...pageProps}/>} {modal==='bookView'&&selected&&<BookView {...pageProps} book={selected}/>} {modal==='dict'&&<DictForm {...pageProps}/>} {modal==='year'&&<YearForm {...pageProps}/>} {modal==='pin'&&<PinForm {...pageProps}/>} {modal==='drive'&&<DriveBackup {...pageProps}/>} {modal==='promotion'&&<PromotionForm {...pageProps}/>} {modal==='scan'&&<Scanner {...pageProps}/>} 
 </div>
}
